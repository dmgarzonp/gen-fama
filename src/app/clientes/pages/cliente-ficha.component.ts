...
export class ClienteFichaComponent implements OnInit {
  ...
  getTipoClass(tipo: string): string {
    switch(tipo) {
      ...
      case 'Crédito': return 'bg-green-100 text-green-800 border-green-200';
      default: return 'bg-gray-100 text-gray-800 border-gray-200';
    }
  }

  getProductosFrecuentes(cliente: Cliente | null): string[] {
    if (!cliente || !cliente.productos_frecuentes) return [];
    if (Array.isArray(cliente.productos_frecuentes)) return cliente.productos_frecuentes;
    if (typeof cliente.productos_frecuentes === 'string') {
      try {
        return JSON.parse(cliente.productos_frecuentes);
      } catch {
        return [];
      }
    }
